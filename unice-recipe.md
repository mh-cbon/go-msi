...
```sh
wget -O go-msi.msi https://github.com/mh-cbon/go-msi/releases/download/0.0.14/go-msi-0.0.14-x64.msi
# or
curl -O https://github.com/mh-cbon/go-msi/releases/download/0.0.14/go-msi-0.0.14-x64.msi
```

Trigger `go-msi` setup on the remote windows machine,

```sh
vagrant winrm -c "msiexec.exe /i C:\\vagrant\\go-msi-0.0.14-x64.msi /quiet"
```
